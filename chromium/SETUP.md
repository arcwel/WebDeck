...
**Budget:** about 50 GB to download and 3 hours to build cold, on this Mac.
Plan for 250 GB free.

## The one rule: use the same path

Put the checkout at **`/Volumes/BG_Dev/webdeck-chromium/chromium/src`**.

`fork.json`, the docs, and seven of the app's scripts default to that path, so
naming the new drive `BG_Dev` makes all of them work unchanged. Format it as
**APFS (Case-sensitive)**: Chromium's tree has files that differ only by case,
and a case-insensitive volume silently collides them.

## Before you wipe the old drive

- **Save the 26.5 SDK.** `/Volumes/BG_Dev/webdeck-chromium/sdks/MacOSX26.5.sdk`
...
```bash
ROOT=/Volumes/BG_Dev/webdeck-chromium
...
```bash
cd "$SRC"
caffeinate -s gclient sync --with_branch_heads --with_tags --force --reset
git fetch --tags
...
```bash
autoninja -C "$SRC/out/webdeck-release" chrome/browser/ui/webui/webdeck:mojo_bindings_ts__generator
...
node scripts/package-fork.mjs --build-dir "$SRC/out/webdeck-release" --out /Volumes/BG_Dev/webdeck-rc1 --keep-stage --identity "Arcwel WebDeck Dev" --allow-dev-keychain
```
...
ditto "/Volumes/BG_Dev/webdeck-rc1/stage/Arcwel WebDeck.app" "$APP.new"
